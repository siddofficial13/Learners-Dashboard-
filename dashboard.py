...
def load_data(link):
    ...
    if selected_section == "Data Cleaning":
        ...
        df_test['Enrollment Time'].fillna(dummy_datetime, inplace=True)
        st.markdown(
            " Filling all the NaN values with the dummy values of Enrollment time by filling them with the time 1 January 1900 and time will be midnight timming.")
        st.dataframe(df_test['Enrollment Time'])
        ...
        st.markdown(
            "Filling all the NaN values with the dummy values of Completion time by filling them with the time 1 January 1900 and time will be midnight timming.")
    ...
    # Section : Data Insights
        ...
        distribution_counts_division_group_department = df.groupby(
            ['Division', 'Group', 'Department']).size().reset_index(
            name='Learner Count')
        ...
        distribution_counts_group_Department = df.groupby(['Group', 'Department']).size().reset_index(
            name='Learner Count')
        distribution_counts_group_Department_sorted = distribution_counts_group_Department.sort_values(
            by='Learner Count',
        ...
        enrolled_by_department = enrolled_learners.groupby('Department').size().reset_index(
            name='Enrolled Learner Count')
        ...
        with col_pro_2:
            ...
            st.markdown(
                "School of Analytics-Basic , School Of Blockchain, 	Project Mgmt Learning Program-Generic, Business Communication Learning Program, Six Sigma Black Belt Learning Program")
            st.markdown("")
            st.markdown("")
        ...
        program_completion_summary_sorted = program_completion_summary.sort_values(by='CompletionRatePercentage',
                                                                                   ascending=False)
        ...
        rounded_completion_rate_dict = dict(
            zip(rounded_completion_rate_counts['CompletionRatePercentage'],
                rounded_completion_rate_counts['Frequency']))
        ...
        higher_completion_courses = course_completion_rates[
            course_completion_rates['CompletionRate'] > threshold_higher]
        ...
        # Sort the DataFrame by 'Average Learning Hours' in descending order
        top_10_courses = average_learning_hours_per_course.sort_values(by='Average Learning Hours',
        ...
        chart_pro = alt.Chart(df_pro).mark_bar().encode(
            x='Completed:N',
            y='proportion:Q',
            color=alt.Color('Completed:N', scale=alt.Scale(scheme='category10')),
            # You can use a different color scheme
        ...
        st.markdown("")
        st.markdown(
            "### Enrollment Time & Completion Time Distribution Graph")  # Convert the datetime columns to year-month format
        df['Enrollment Time'] = pd.to_datetime(df['Enrollment Time'], errors='coerce')
        df['Completion Time'] = pd.to_datetime(df['Completion Time'], errors='coerce')
        ...
        for x, y_enroll, y_complete in zip(plot_data.index.astype(str), plot_data['Enrollment'],
        ...
        st.markdown("")
        st.markdown(
            "Above Plot shows overall trend in the enrollment and the completeion time by the learners who are enrolled in the courses ")
        st.markdown("")
        st.markdown("")
        ...
        st.metric(label="Correlation Between Course Grade & Duration For Not Completed Courses",
                  value=correlation_not_completed_round, delta=None)
        st.markdown("")
        st.markdown("")
        ...
        st.markdown(
            "There is a moderate positive correlation between learning hours and course grades for learners who have not completed their courses. As learning hours increase, there is a tendency for grades to improve.")
        st.markdown("")
        st.markdown("")
        st.markdown("#### Correlation for Completed Courses: 0.01")
        st.markdown("##### Interpretation")
        st.markdown(
            "There is a very weak positive correlation between learning hours and course grades for learners who have completed their courses. The relationship between learning hours and grades for completed courses is nearly negligible.")
        st.markdown("")
        st.markdown("")
        st.markdown("#### Correlation for All Courses: 0.27")
        st.markdown("##### Interpretation")
        st.markdown(
            "When considering all courses, including both completed and not completed, there is a moderate positive correlation between learning hours and course grades. The overall relationship suggests that, on average, more learning hours are associated with higher grades.")
        st.markdown("")
